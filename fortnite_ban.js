...
const { Presence, MessageEmbed, Collection, TextChannel } = require("discord.js")
...
module.exports = async function (_, presence) {
    ...
    if (!presence.member.bannable) {
        if (!channel) return
        if (!channel.permissionsFor(presence.guild.me).serialize().SEND_MESSAGES) return
        const embed = new MessageEmbed()
            .setAuthor(presence.member.user.tag, presence.member.user.displayAvatarURL({ dynamic: true, size: 256 }))
            .setDescription("is playing Fortnite but I cannot ban it!")
            .setColor(0xff8800)
            .setTimestamp()
            .setFooter("Don't play Fortnite!", presence.member.client.user.displayAvatarURL({ dynamic: true, size: 256 }))
        channel.send(embed)
        return
    } else {
        try {
            await presence.member.send("You are being banned from " + presence.guild.name + " for playing Fortnite.").catch(() => { })
            await presence.member.ban({ reason: "Playing Fortnite" }).then(async member => {
                ...
                let guildBanCount = await stats.get(member.guild.id + "-ban-count") || 0
                guildBanCount += 1
                stats.set(member.guild.id + "-ban-count", guildBanCount)
                if (!channel) return
                if (!channel.permissionsFor(presence.guild.me).serialize().SEND_MESSAGES) return
                const embed = new MessageEmbed()
                    .setAuthor(presence.user.tag, presence.user.displayAvatarURL({ dynamic: true, size: 256 }))
                    .setDescription("is banned for playing Fortnite!")
                    .setColor(0xff0000)
                    .setTimestamp()
                    .setFooter("Don't play Fortnite!", presence.user.client.user.displayAvatarURL({ dynamic: true, size: 256 }))
                channel.send(embed)
                return
            }).catch(console.error)
        } catch (error) {
            console.log("It seems that someone else kicked on banned the member before me!")
        }
    }
}